...
class PhpNamespaceCopyCommand(sublime_plugin.WindowCommand):
    def run(self):
        if is_php_file(self.window.active_view()):
            ...
            sublime.status_message('Copied: ' + get_namespace(self.window))
...
class PhpNamespaceCopyUseCommand(sublime_plugin.WindowCommand):
    def run(self):
        if is_php_file(self.window.active_view()):
            sublime.set_clipboard('use ' + get_namespace(self.window) + ';')
            sublime.status_message('Copied: ' + get_namespace(self.window))
            close_overlay(self.window, currentView)
